...
The fix is to take all three decisions away from in-the-moment judgment. The scripts `scripts/wt.sh` (bash / Git Bash / WSL / macOS / Linux) and `scripts/wt.ps1` (PowerShell 5.1+) compute the path, sanitize the name, and refuse to delete unsaved work.
...
WT=$(scripts/wt.sh new "login-oauth" --task "issue #412")
cd "$WT"
```

```powershell
# PowerShell
$wt = .\scripts\wt.ps1 new "login-oauth" --task "issue #412"
Set-Location $wt
```
...
```bash
WT=$(scripts/wt.sh new fix-412 --from main --task "crash on sign-out")
cd "$WT"
```
...
```bash
scripts/wt.sh rm fix-412                    # removes the worktree, keeps the branch
scripts/wt.sh rm fix-412 --delete-branch    # also deletes the local branch
```
...
```bash
scripts/wt.sh doctor          # reports, changes nothing
scripts/wt.sh doctor --fix    # relocates misplaced ones, removes safe orphans
```
...
```bash
scripts/wt.sh clean                        # dry run: shows what it would delete
scripts/wt.sh clean --merged --yes         # deletes only clean, already-merged ones
scripts/wt.sh clean --older-than 14 --yes  # and only those older than 14 days
```

`clean` without `--yes` deletes nothing. **Show the dry-run output to the user before running the real deletion**, especially the first time in a repo: there may be work there that only they can judge.
